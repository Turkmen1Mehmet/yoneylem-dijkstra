...
def myFunc(filepath):
    ...
    plt.title('DİJKSTRA GÖRSEL ŞÖLEN', size=15)
    ...
    with open(filepath, mode='r') as csv_file:
        ...
        weight = []
        colors = ['red', 'yellow', 'violet', 'orange', 'red', 'yellow', 'violet', 'yellow', 'red', 'blue', 'orange', 'red','yellow', 'violet','red']
        for row in csv_reader:
            #appending the paths from the index 3 till end of row
            A.append(row[3:])
            if len(row[2])<=3 :#to omit DBL_MAX instead use 1000 to represent infinity
        ...
        for i in A:
            ...
            for b in B:
                ...
                M.append(g.nodes[b]['X'])
                N.append(g.nodes[b]['Y'])
            x = x+1
